...
// DHT definitions
#define DHTPIN 12
#define DHTTYPE DHT22 // Using DHT11
...
void navigate(){  // Hug left wall
  ...
  if (state == forward){
    ...
    else if (backingOut && right_min >= 13 && center_min < 13) turnRight();
    else {
      goForward();
      backingOut = 0;
    }
...
void findCorner(){
  ...
  if (targetX - posX < 0){
    if (target_angle < 0) target_angle = pi + target_angle;
    else target_angle = -1*pi + target_angle;
  }
  Serial.print("Target Angle: ");
  Serial.println(target_angle);
  ...
  if (checking_state == 2){
    ...
    if (posX > (targetX - 1) && posX < (targetX + 1) && posY > (targetY - 1) && posY < (targetY + 1)){
      ...
      if (state == forward) stuck_counter = 0;
      goForward();
    }
    else {
      checking_state = 3;
